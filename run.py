...
def str_del(str, i):
    return str[:i] + (str[i + 1:] if i + 1 < len(str) else [])
        

def str_insert(str, i, ins):
    return str[:i] + ins + (str[i:] if i + 1 < len(str) else [])
        
        
def add_constraint_list(str):
    logic = "()|&~>< "
    prop_strs = []
    current_var = ""
    i = 0
    while i < len(str):
        if str[i] not in logic:
            while str[i] not in logic:
                current_var += str[i]
                str = str_del(str, i)
                if i >= len(str):
                    break
            prop_strs.append(current_var)
            str = str_insert(str, i, "prop_dict[" + current_var + "][0]")
            i += len("prop_dict[" + current_var + "][0]")
            current_var = ""
        i += 1
    prop_lis_lis = []
    rec(prop_strs, prop_lis_lis)
    for prop_lis in prop_lis_lis:
        E.add_constraint(eval(str))
...
def example_theory():
    ...
    add_constraint_list("((m_nq | f_sc | e_ns) & ~(m_nq & f_sc & e_ns) & ~(m_nq & f_sc) & ~(f_sc & e_ns) & ~(m_nq & e_ns))")
